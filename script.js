...
// Гістограма (стовпчикова діаграма) — використовується для телеметрії БПЛА
// замість лінійного графіка. Кожне історичне значення показане окремим стовпчиком.
function drawHistogram(id, values, options = {}) {
  const canvas = $(id);
  const prepared = prepareCanvas(canvas);
  if (!prepared) return;
  const {ctx, width, height} = prepared;
  ...
  const colorMap = {
    ...
    orange: 'rgba(251,146,60,.85)',
    red: 'rgba(255,49,88,.85)',
    purple: 'rgba(192,132,252,.85)',
    green: 'rgba(0,255,136,.85)',
    blue: 'rgba(56,189,248,.85)',
  };
  ...
  clean.forEach((v, i) => {
    ...
    const barH = Math.max(1, pct * height);
    const x = i * (barW + gap);
...
function updateDrone(drone, history) {
  ...
  // Телеметрія БПЛА: гістограма замість лінійного графіка
  drawHistogram('drCurrentChart', history.map(x => x.Icurrent), {min: 0, max: 45, color: 'cyan'});
  drawHistogram('drTempChart', history.map(x => x.Tcurrent), {min: 20, max: 100, color: 'orange'});
  drawHistogram('drRiskChart', history.map(x => x.risk), {min: 0, max: 1, color: 'purple'});
  drawHistogram('drHeatChart', history.map(x => x.electrical_power_W), {min: 0, max: 1100, color: 'cyan'});
  drawHistogram('drVoltageChart', history.map(x => x.voltage_V), {min: 18, max: 26, color: 'green'});
  drawHistogram('drRpmChart', history.map(x => x.rpm), {min: 0, max: 20000, color: 'blue'});
  drawHistogram('drMagChart', history.map(x => x.magnet_health_percent), {min: 0, max: 100, color: 'green'});
  drawHistogram('drEffChart', history.map(x => x.efficiency_percent), {min: 40, max: 90, color: 'orange'});
  drawHistogram('drHeatRateChart', history.map(x => x.heating_rate_C_per_s), {min: 0, max: 2, color: 'red'});
  drawHistogram('drCoolRateChart', history.map(x => x.cooling_rate_C_per_s), {min: 0, max: 2, color: 'cyan'});
}
